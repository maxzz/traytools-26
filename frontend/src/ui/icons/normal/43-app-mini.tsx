...
export function IconAppMini({ className, title, fillClasses, isMini, ...rest }: HTMLAttributes<SVGSVGElement> & {fillClasses?: string; isMini?: boolean}) {
    const top = isMini ? "4.08" : "5.08";
    const height = isMini ? "7.4" : "10.63";
    return (
        <svg className={classNames("fill-none stroke-current stroke-[1.5px]", className)} strokeLinecap="round" strokeLinejoin="round" viewBox="0 0 24 24" {...rest}>
            ...
            <rect className={fillClasses} x="7.51" y={top} width="14.63" height={height} rx="1.81" ry="1.81"/>
        </svg>
    );
}
